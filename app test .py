...
.stButton > button {
    background: linear-gradient(135deg, var(--accent), #9b59f7) !important;
    color: #fff !important;
...
with st.sidebar:
    ...
    <div style='text-align:center; padding: .5rem 0 1.5rem'>
        ...
        <div style='font-family:"DM Serif Display",serif; font-size:1.4rem; color:#e8eaf0'>MindCheck</div>
        <div style='font-size:.75rem; color:#8b90a8; letter-spacing:.08em; text-transform:uppercase'>Student Wellbeing AI</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown("<hr style='border-color:#2e3350; margin:1.2rem 0'>", unsafe_allow_html=True)
    st.markdown("""
    <div style='font-size:.78rem; color:#8b90a8; line-height:1.7'>
    ...
    if not model_loaded:
        st.markdown("""
        <div style='background:#2a1f1f;border:1px solid #6b3333;border-radius:10px;padding:.8rem 1rem;margin-top:1rem;font-size:.78rem;color:#f87171'>
...
if "🔍" in page:
    ...
    if submitted:
        ...
        if risk_pct >= 70:
            ring_color = "#f87171"; badge_bg = "#2a1a1a"; badge_fg = "#f87171"; level_text = "High Risk"
        elif risk_pct >= 40:
            ring_color = "#fbbf24"; badge_bg = "#2a2213"; badge_fg = "#fbbf24"; level_text = "Moderate Risk"
        else:
            ring_color = "#34d399"; badge_bg = "#142a22"; badge_fg = "#34d399"; level_text = "Low Risk"
        ...
        <div class='mc-card' style='text-align:center; padding:2rem'>
            ...
            <div class='score-wrap'>
                <div class='score-ring' style='
                    ...
                    box-shadow: 0 0 40px {ring_color}30;
                '>{risk_pct}%</div>
                <span class='risk-badge' style='background:{badge_bg}; color:{badge_fg}; border:1px solid {ring_color}40'>{level_text}</span>
            </div>
            <p style='color:#8b90a8; font-size:.9rem; margin-top:1rem; max-width:420px; margin-left:auto; margin-right:auto; line-height:1.65'>
...
# ─────────────────────────────────────────────
    ...
    <p style='color:#8b90a8; max-width:600px; line-height:1.7; font-size:1rem; margin-top:.5rem'>
    ...
    <p style='color:#8b90a8; font-size:.9rem; line-height:1.7; margin-top:.5rem'>
    ...
    rows = "".join([
        f"<tr><td style='padding:.55rem .8rem; color:#e8eaf0'>{n}</td><td style='padding:.55rem .8rem; color:#8b90a8'>{t}</td><td style='padding:.55rem .8rem; color:#8b90a8'>{e}</td></tr>"
        for n, t, e in feat_table
    ])
    st.markdown(f"""
    <div class='mc-card'>
    <table style='width:100%; border-collapse:collapse; font-size:.85rem'>
        <thead><tr style='border-bottom:1px solid #2e3350'>
            <th style='padding:.6rem .8rem; text-align:left; color:#a78bfa; font-weight:600'>Feature</th>
            <th style='padding:.6rem .8rem; text-align:left; color:#a78bfa; font-weight:600'>Type</th>
            <th style='padding:.6rem .8rem; text-align:left; color:#a78bfa; font-weight:600'>Encoding</th>
        </tr></thead>
        <tbody>{rows}</tbody>
    </table>
    </div>
    ...
    <p style='color:#8b90a8; font-size:.9rem; line-height:1.7; margin-top:.5rem'>
    Based on the dataset type and binary classification task, the model was most likely trained using a <b style='color:#e8eaf0'>Random Forest</b> classifier — an ensemble of decision trees that votes on the final prediction. Random Forests handle mixed numeric/ordinal features well, are robust to outliers, and naturally provide feature importances.
    Other possibilities include <b style='color:#e8eaf0'>Logistic Regression</b> (fast, interpretable baseline), <b style='color:#e8eaf0'>Gradient Boosting / XGBoost</b> (higher accuracy, slower), or <b style='color:#e8eaf0'>SVM</b> (good on small datasets with clear boundaries).
    </p>
    </div>
...
# ─────────────────────────────────────────────
    ...
    for emoji, name, contact, hours, desc in resources:
        ...
        <div class='mc-card' style='display:flex; gap:1.2rem; align-items:flex-start'>
            ...
            <div>
                <b style='font-size:1rem; color:#e8eaf0'>{name}</b>
                <div style='font-size:.88rem; color:#a78bfa; margin:.2rem 0'>{contact}</div>
                <div style='font-size:.78rem; color:#8b90a8'>{hours} &nbsp;·&nbsp; {desc}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
    ...
    <div style='display:grid; grid-template-columns:1fr 1fr; gap:1rem; margin-top:1rem'>
        <div class='tip-pill'><b>📦 Box breathing</b><br><span style='color:#8b90a8;font-size:.85rem'>Inhale 4s → Hold 4s → Exhale 4s → Hold 4s. Repeat 4×. Reduces acute anxiety in under 5 minutes.</span></div>
        <div class='tip-pill'><b>🌿 5-4-3-2-1 grounding</b><br><span style='color:#8b90a8;font-size:.85rem'>Name 5 things you see, 4 you feel, 3 you hear, 2 you smell, 1 you taste. Anchors you to the present.</span></div>
        <div class='tip-pill'><b>📓 Journaling</b><br><span style='color:#8b90a8;font-size:.85rem'>Write for 10 minutes without editing. Studies show expressive writing reduces depression symptoms.</span></div>
        <div class='tip-pill'><b>🚶 Nature walks</b><br><span style='color:#8b90a8;font-size:.85rem'>20 minutes in green space measurably lowers cortisol. No gym needed.</span></div>
        <div class='tip-pill'><b>📵 Phone-free mornings</b><br><span style='color:#8b90a8;font-size:.85rem'>The first 30 minutes without your phone sets a calmer baseline for the whole day.</span></div>
        <div class='tip-pill'><b>🤝 Peer support</b><br><span style='color:#8b90a8;font-size:.85rem'>Simply telling one person how you feel — even a message — reduces psychological burden immediately.</span></div>
    </div>
    </div>
    """, unsafe_allow_html=True)
...
# ─────────────────────────────────────────────
    ...
    st.markdown("""
    <div class='mc-card mc-card-accent'>
    <p style='color:#8b90a8; font-size:.95rem; line-height:1.75'>
    MindCheck was built to give students a <b style='color:#e8eaf0'>private, non-judgemental first step</b> toward understanding their mental health.
    ...
    <h2>Tech stack</h2>
    <p style='color:#8b90a8; font-size:.9rem; line-height:1.7; margin-top:.5rem'>
    Built with <b style='color:#e8eaf0'>Python 3</b>, <b style='color:#e8eaf0'>Streamlit</b> for the UI, <b style='color:#e8eaf0'>scikit-learn</b> for the ML model, and <b style='color:#e8eaf0'>NumPy</b> for feature processing. The model is loaded from a pre-trained <code>.pkl</code> file serialised via pickle.
    </p>
    </div>
